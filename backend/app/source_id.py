"""source_id.py — identify the SOURCE of a public statement from a quote + metadata.

Web-searches (locally) over a quote/description, then asks the LLM to name the
speaker, event, and a query to fetch the video. Returns cited results. Never
identifies a person from their face — only from the words and citations.

    python -m app.source_id "ask not what your country can do for you"
"""
from __future__ import annotations

import argparse
import json
from typing import Optional

from . import llm, websearch
from .logging_setup import get_logger

log = get_logger(__name__)

_SYSTEM = (
    "You identify the SOURCE of a public statement (speech/interview/press conference) "
    "from a quote or description. You identify the SPEAKER and the EVENT, never a person "
    "from their face. You are given web search results — rely on them; do not invent a "
    "source you cannot support.\n"
    "Reply with a fenced ```json block with exactly these keys:\n"
    '{"source_title": string|null, "context_summary": string, '
    '"confidence": "high"|"medium"|"low", "search_terms": string, '
    '"candidate_video_query": string|null}\n'
    "If the results don't clearly identify a source: source_title=null, confidence=\"low\"."
)


def identify_source(query: str, metadata: Optional[dict] = None) -> dict:
    results = websearch.search(f"speech interview press conference quote {query}", max_results=5)
    user = f"Statement query / quote: {query}\n"
    if metadata:
        meta = ", ".join(f"{k}={v}" for k, v in metadata.items() if v)
        if meta:
            user += f"Known metadata: {meta}\n"
    user += "\nWeb search results:\n" + websearch.format_results(results)
    user += "\n\nIdentify the source and return the JSON."

    resp = llm.chat([{"role": "system", "content": _SYSTEM}, {"role": "user", "content": user}])
    data = llm.extract_json(resp.text) or {}
    citations = [{"title": r["title"], "url": r["url"], "cited_text": r["snippet"][:200]} for r in results]
    result = {
        "source_title": data.get("source_title"),
        "context_summary": (data.get("context_summary") or "").strip(),
        "confidence": data.get("confidence", "low"),
        "search_terms": data.get("search_terms") or query,
        "candidate_video_query": data.get("candidate_video_query"),
        "citations": citations,
    }
    log.info("source_id: %s (confidence=%s)", result["source_title"], result["confidence"])
    return result


def _cli() -> None:
    ap = argparse.ArgumentParser(description="Identify a public statement's source")
    ap.add_argument("query")
    args = ap.parse_args()
    print(json.dumps(identify_source(args.query), indent=2))


if __name__ == "__main__":
    _cli()
